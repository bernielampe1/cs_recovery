...
def omp_test():
    ...
    print(f"calling omp(), k = {k}")

    D = fft.dct(np.eye(A.shape[1]), norm='ortho', axis=0)  # optimize over the dct domain
    x_hat = omp(y, np.dot(A, D.T), sparsity_term, k)       # omp call with sparsity parameter
    ...
    p = 0.99
    print(f"calling omp(), p = {p}")

    # omp call with percentage parameter
    D = fft.dct(np.eye(A.shape[1]), norm='ortho', axis=0) # optimize over the dct domain
    x_hat = omp(y, np.dot(A, D.T), percent_term, p)       # omp call with percent parameter
    plt_error(x_f, x_hat)                                 # print and graph errors
    ...
    e = 0.00001
    print(f"calling omp(), e = {e}")

    # omp call with percentage parameter
    D = fft.dct(np.eye(A.shape[1]), norm='ortho', axis=0) # optimize over the dct domain
    x_hat = omp(y, np.dot(A, D.T), epsilon_term, e)       # omp call with epsilon parameter
    plt_error(x_f, x_hat)                                 # print and graph errors
